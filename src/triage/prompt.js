...
export function buildTriagePrompt({ source, options = {} }) {
  ...
  return [
    ...
    "- Recommend exactly one attention level: Discard, Skim, Focused read, or Deep study.",
    ...
    `- The result may be saved to ${ledgerPath}, but do not call research_ledger_append in the initial report.`,
    "- Only call research_ledger_append after the user explicitly asks to save or record this result.",
    "- When saving, pass the source, attention tier, next action, and the complete triage report as the ledger content.",
    "",
    "Use this report structure:",
    "## Decision",
    "- Attention: Discard | Skim | Focused read | Deep study",
    "- Reason:",
